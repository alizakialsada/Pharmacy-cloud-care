-- Pharmacy cloud care: add auditable cancellation (VOID) of dispensing records.
-- Run this ONCE in Supabase SQL Editor after the original production SQL.
...
declare
  uid uuid;
  fname text;
  rec public.medica_dispense_log%rowtype;
begin
  ...
  if not found then
    raise exception 'Dispensing record not found or already cancelled';
  end if;
