...
-- === Auditable Dispense Cancellation / VOID support ===
...
create or replace function public.medica_void_dispense(p_token text,p_dispense_id bigint,p_reason text)
...
begin
  ...
  update public.medica_dispense_log set is_void=true,voided_at=now(),voided_by_id=uid,voided_by_name=fname,void_reason=trim(p_reason)
   where id=p_dispense_id and is_void=false returning * into rec;
...
begin
  perform public.medica_valid_pharmacist(p_token);
  return coalesce((select jsonb_agg(to_jsonb(x) order by x.dispensed_at desc) from (
    select id,dispensed_at,pharmacist_name,hospital,pharmacy,mrn,patient_name,department,medication,dosage_form,dose,frequency,route,quantity,supply_days,directions_en,directions_ar,label_type,
           is_void,voided_at,voided_by_name,void_reason
    from public.medica_dispense_log where hospital=p_hospital and mrn=p_mrn
  ) x),'[]'::jsonb);
end $$;
